...
Exporter exactement ce qu'on veut : _"Tous mes picks"_, _"Photos 4 étoiles et +"_, _"Tout sauf les rejetées"_. Bout logique du pipeline de tri.
    ...
    <div className="flex flex-wrap items-center gap-2">
      {PRESETS.map((p) => (
        <button
          ...
          className={
            isActive(current, p.filter) ? 'ring-2 ring-primary ...' : '...'
          }
        >
          {p.label}
        </button>
...
- Toast de confirmation : _"Métadonnées copiées"_ / _"Collées sur 12 photos"_
...
Savoir en un coup d'œil où on en est : _"47 / 120 triées"_, _"12 picks · 8 rejetées · 27 sans note"_. Sentiment de progression, motivation.
...
export function SessionProgress({ photos }: { photos: Photo[] }) {
  ...
  const pct =
    analyzed.length > 0 ? Math.round((actioned / analyzed.length) * 100) : 0;

  return (
    <div className="space-y-2">
      <div className="flex items-center justify-between text-xs text-muted-foreground">
        <span>
          {actioned} / {analyzed.length} triées ({pct}%)
        </span>
...
export function RenamePanel({ photos, onChange }) {
  ...
  return <Collapsible>{/* Pattern input + tokens + preview */}</Collapsible>;
}
```
...
export function AutoRateModal({ open, onOpenChange, photos }) {
  const [preset, setPreset] = useState<'strict' | 'balanced' | 'generous'>(
    'balanced'
  );
  ...
  const distribution = useMemo(
    () => computeDistributionPreview(photos, preset),
    [photos, preset]
  );
...
export function SurveyView({ photos, open, onClose, onSelectBest }) {
  ...
  return (
    <div className="fixed inset-0 z-[300] flex flex-col bg-black">
      <div className={`grid flex-1 ${cols} gap-1 p-2`}>
...
function SurveyCell({ photo, index, isWinner, onClick }) {
  return (
    <div
      className={`relative cursor-pointer overflow-hidden rounded transition-all ${
        isWinner
    ...
    >
      ...
      {/* Badge index */}
      <div className="absolute left-2 top-2 rounded bg-black/70 px-1.5 py-0.5 text-xs font-bold text-white">
        {index}
      </div>
      {/* Badge winner */}
      {isWinner && (
        <div className="absolute right-2 top-2 rounded bg-green-500 px-2 py-0.5 text-xs font-medium text-white">
          ✓ Meilleure
        </div>
      )}
      {/* Note + pick overlay */}
...
_Ce document est vivant. Mettre à jour le tableau de statut au fil des implémentations._
