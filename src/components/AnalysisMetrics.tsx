...
export function AnalysisMetrics() {
  ...
  const sharpnessScores = photosWithAnalysis.map((photo) => photo.analysis?.sharpnessScore || 0);
  const brightnessScores = photosWithAnalysis.map((photo) => photo.analysis?.suggestedRetouch?.brightness || 0);
  const contrastScores = photosWithAnalysis.map((photo) => photo.analysis?.suggestedRetouch?.contrast || 0);
  const saturationScores = photosWithAnalysis.map((photo) => photo.analysis?.suggestedRetouch?.saturation || 0);
  ...
  const avgSaturation = saturationScores.reduce((a, b) => a + b, 0) / saturationScores.length;

  const blurryCount = photosWithAnalysis.filter((photo) => photo.analysis?.isBlurry).length;
  const sharpCount = photosWithAnalysis.filter((photo) => !photo.analysis?.isBlurry).length;
  const eyesDetectedCount = photosWithAnalysis.filter((photo) => photo.analysis?.hasOpenEyes).length;
      ...
      <CardContent className="space-y-4">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
          <div className="text-center">
            <div className="text-2xl font-bold text-blue-600">{photosWithAnalysis.length}</div>
            <div className="text-sm text-gray-600">Photos analysées</div>
          </div>
          <div className="text-center">
            <div className="text-2xl font-bold text-green-600">{sharpCount}</div>
          ...
          <div className="text-center">
            <div className="text-2xl font-bold text-purple-600">{eyesDetectedCount}</div>
        ...
        <div className="space-y-3">
          <h4 className="font-semibold text-sm">Moyennes des scores</h4>
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
            <div>
              <div className="text-sm text-gray-600">Netteté</div>
              <div className="text-lg font-semibold">{Math.round(avgSharpness * 100)}%</div>
            </div>
            <div>
              <div className="text-sm text-gray-600">Luminosité</div>
              <div className="text-lg font-semibold">{Math.round(avgBrightness * 100)}%</div>
            </div>
            <div>
              <div className="text-sm text-gray-600">Contraste</div>
              <div className="text-lg font-semibold">{Math.round(avgContrast * 100)}%</div>
            </div>
            <div>
              <div className="text-sm text-gray-600">Saturation</div>
              <div className="text-lg font-semibold">{Math.round(avgSaturation * 100)}%</div>
            </div>
          </div>
        </div>
        ...
        <div className="space-y-2">
          <h4 className="font-semibold text-sm">Distribution de la netteté</h4>
          <div className="space-y-2">
            <div className="flex items-center justify-between">
              ...
              <div className="flex items-center gap-2">
                ...
                <span className="text-sm font-medium">{sharpnessDistribution.verySharp}</span>
              </div>
            </div>
            <div className="flex items-center justify-between">
              ...
              <div className="flex items-center gap-2">
                ...
                <span className="text-sm font-medium">{sharpnessDistribution.sharp}</span>
              </div>
            </div>
            <div className="flex items-center justify-between">
              ...
              <div className="flex items-center gap-2">
                ...
                <span className="text-sm font-medium">{sharpnessDistribution.moderate}</span>
              </div>
            </div>
            <div className="flex items-center justify-between">
              ...
              <div className="flex items-center gap-2">
                ...
                <span className="text-sm font-medium">{sharpnessDistribution.blurry}</span>
              </div>
            </div>
          </div>
        </div>
        ...
        <div className="space-y-2">
          <h4 className="font-semibold text-sm">Tags les plus fréquents</h4>
