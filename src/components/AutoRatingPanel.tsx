    ...
    try {
      ...
      toast.success(
        <div className="space-y-1">
          <div className="font-semibold">✨ {analyzedPhotos.length} photos notées automatiquement</div>
  ...
  return (
    <Card className="w-full">
      ...
      <CardContent className="space-y-4">
        {/* Info */}
        <div className="flex items-start gap-2 p-3 bg-info/10 border border-info/20 rounded-lg text-sm">
        ...
        >
          ...
          {isProcessing ? 'Notation en cours...' : `Noter automatiquement ${analyzedPhotos.length} photo${analyzedPhotos.length > 1 ? 's' : ''}`}
        </Button>
        ...
        <div className="p-3 bg-muted/50 rounded-lg space-y-2">
          ...
          <div className="space-y-1 text-xs">
            <div className="flex items-center gap-2">
              ...
              <span className="text-muted-foreground">Score ≥ 90% - Excellente</span>
            </div>
            <div className="flex items-center gap-2">
              <span className="text-yellow-500">⭐⭐⭐⭐</span>
              <span className="text-muted-foreground">Score ≥ 75% - Très bonne</span>
            </div>
            <div className="flex items-center gap-2">
              <span className="text-yellow-500">⭐⭐⭐</span>
            ...
            <div className="flex items-center gap-2">
              <span className="text-yellow-500">⭐⭐</span>
              <span className="text-muted-foreground">Score ≥ 40% - Moyenne</span>
            </div>
            <div className="flex items-center gap-2">
              <span className="text-yellow-500">⭐</span>
              <span className="text-muted-foreground">Score ≥ 20% - Faible</span>
            </div>
            <div className="flex items-center gap-2">
              <span className="text-muted-foreground">☆</span>
              <span className="text-muted-foreground">Score &lt; 20% - À rejeter</span>
            </div>
          </div>
        </div>
        ...
        <p className="text-xs text-muted-foreground text-center">
          💡 Vous pouvez toujours ajuster manuellement les notes après la notation automatique
        </p>
      </CardContent>
    </Card>
  );
