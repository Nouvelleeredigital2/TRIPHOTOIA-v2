...
function ExifRow({ label, value }: { label: string; value: string }) {
  return (
    <div className="flex justify-between gap-2">
      ...
      <span
        className="max-w-[12rem] truncate text-right text-white"
        title={value}
      >
        {value}
      </span>
    ...
    const handleKeyDown = (e: KeyboardEvent) => {
      ...
      switch (e.key) {
        ...
        case '5': {
          ...
          toast.success(
            n === 0 ? 'Note retirée' : `${n} étoile${n > 1 ? 's' : ''}`
          );
          break;
        }
        case '6':
        ...
        case 'P':
          ...
          toast.success(
            s.photo.analysis?.isPick ? 'Pick retiré' : '🎯 Marqué comme Pick'
          );
          break;
        case 'x':
        case 'X':
          ...
          toast.success(
            s.photo.analysis?.isRejected ? 'Reject retiré' : '❌ Photo rejetée'
          );
          break;
        case 'u':
        case 'U':
  ...
  return (
    <AnimatePresence>
      ...
      >
        ...
        <div className="absolute left-0 right-0 top-0 z-10 bg-gradient-to-b from-black/80 to-transparent p-4">
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-4">
              <h3 className="font-medium text-white">{photo.file.name}</h3>
              ...
              {photos.length > 1 && (
                <div className="flex items-center gap-1 rounded-lg bg-white/10 px-2 py-1">
                  ...
                  <Button
                    size="sm"
                    variant="ghost"
                    className={`h-7 w-7 p-0 text-white hover:bg-white/20 ${slideshowActive ? 'bg-white/20' : ''}`}
                    onClick={() => setSlideshowActive((v) => !v)}
                    title={
                      slideshowActive
                        ? 'Pause diaporama (Espace)'
                        : 'Lancer le diaporama (Espace)'
                    }
                  >
                    {slideshowActive ? (
        ...
        <div
          ref={containerRef}
          className="absolute inset-0 flex items-center justify-center overflow-hidden p-16"
          style={{
        ...
        {hasPrevious && (
          <Button
            size="lg"
            variant="ghost"
            className="absolute left-4 top-1/2 h-16 w-16 -translate-y-1/2 text-white hover:bg-white/20"
            onClick={onPrevious}
          >
        ...
        {hasNext && (
          <Button
            size="lg"
            variant="ghost"
            className="absolute right-4 top-1/2 h-16 w-16 -translate-y-1/2 text-white hover:bg-white/20"
            onClick={onNext}
          >
              ...
              <div className="mx-auto max-w-7xl">
                <div className="grid grid-cols-3 gap-8">
                  ...
                  <div className="space-y-3">
                    ...
                    <div className="rounded-lg bg-black/60 p-3 backdrop-blur-sm">
                      <StarRating
                        ...
                        onRatingChange={(rating) => {
                          ...
                          toast.success(
                            `${rating === 0 ? 'Note retirée' : `${rating} étoile${rating > 1 ? 's' : ''}`}`
                          );
                        }}
                        size="lg"
                        showCount
                    ...
                    <div className="flex gap-2">
                      ...
                      <Button
                        size="sm"
                        variant={
                          photo.analysis?.isRejected ? 'destructive' : 'outline'
                        }
                        className="flex-1"
                        onClick={() => {
                  ...
                  <div className="space-y-3">
                    ...
                    <div className="space-y-1.5 text-xs text-white/80">
                      ...
                      <ExifRow
                        label="Taille"
                        value={`${(photo.file.size / 1024 / 1024).toFixed(2)} Mo`}
                      />
                      <ExifRow
                        label="Format"
                        value={
                      ...
                      {photo.metadata?.width && photo.metadata?.height && (
                        <ExifRow
                          label="Dimensions"
                          value={`${photo.metadata.width} × ${photo.metadata.height} px`}
                        />
                      )}
                      {/* Analyse IA */}
                      {photo.analysis?.sharpnessScore !== undefined && (
                        <ExifRow
                          label="Netteté"
                          value={`${Math.round(photo.analysis.sharpnessScore * 100)}%`}
                        />
                      )}
                      {photo.analysis?.compositionScore !== undefined && (
                        <ExifRow
                          label="Composition"
                          value={`${Math.round(photo.analysis.compositionScore * 100)}%`}
                        />
                      )}
                      {photo.analysis?.isBlurry !== undefined && (
                        <ExifRow
                          label="Flou"
                          value={photo.analysis.isBlurry ? '❌ Oui' : '✅ Non'}
                        />
                      )}
                      {/* EXIF caméra */}
                      {photo.metadata?.exif &&
                          ...
                          return (
                            <>
                              ...
                              {e.FocalLength && (
                                <ExifRow
                                  label="Focale"
                                  value={`${e.FocalLength} mm`}
                                />
                              )}
                              {e.ExposureTime && (
                                <ExifRow
                                  label="Vitesse"
                                  value={`1/${Math.round(1 / Number(e.ExposureTime))} s`}
                                />
                              )}
                              {e.FNumber && (
                                <ExifRow
                              ...
                              {e.ISOSpeedRatings && (
                                <ExifRow
                                  label="ISO"
                                  value={String(e.ISOSpeedRatings)}
                                />
                              )}
                              {e.DateTimeOriginal && (
                                <ExifRow
                                  label="Date"
                                  value={String(e.DateTimeOriginal).replace(
                                    'T',
                                    ' '
                                  )}
                                />
                  ...
                  {/* Zoom + Histogramme */}
                  <div className="space-y-3">
                    <h4 className="text-sm font-semibold text-white">Zoom</h4>
                    <div className="flex items-center gap-2">
                      <Button
                        size="sm"
                        variant="outline"
                ...
                <div className="mt-4 border-t border-white/20 pt-4">
                  <div className="flex flex-wrap gap-4 text-xs text-white/60">
                    ...
                    <span>
                      <kbd className="rounded bg-white/10 px-2 py-1">0-5</kbd>{' '}
                      Noter
                    </span>
                    <span>
                      <kbd className="rounded bg-white/10 px-2 py-1">6-9</kbd>{' '}
                      Label couleur
                    </span>
                    <span>
                      <kbd className="rounded bg-white/10 px-2 py-1">P</kbd>{' '}
                      Pick
                    </span>
                    <span>
                      <kbd className="rounded bg-white/10 px-2 py-1">X</kbd>{' '}
                      Reject
                    </span>
                    <span>
                      <kbd className="rounded bg-white/10 px-2 py-1">U</kbd>{' '}
                      Unflag
                    </span>
                    <span>
                      <kbd className="rounded bg-white/10 px-2 py-1">I</kbd>{' '}
                      Infos
                    </span>
                    <span>
                      <kbd className="rounded bg-white/10 px-2 py-1">+/-</kbd>{' '}
                    ...
                    <span>
                      <kbd className="rounded bg-white/10 px-2 py-1">Drag</kbd>{' '}
                      Pan
                    </span>
                    <span>
                      <kbd className="rounded bg-white/10 px-2 py-1">
                    ...
                    <span>
                      <kbd className="rounded bg-white/10 px-2 py-1">F/ESC</kbd>{' '}
                      Quitter
                    </span>
                  </div>
                </div>
