...
export const AutoFlowDupCompare: React.FC<AutoFlowDupCompareProps> = ({
  photos, onDecision, onBack,
}) => {
  const groups = useMemo<DupGroup[]>(() => {
    const map = new Map<string, AfPhoto[]>();
    photos.filter((p) => p.isDup && p.dupGroup).forEach((p) => {
    ...
    <div style={{
      ...
      <div style={{
        ...
        <div style={{ textAlign: 'center' }}>
          <div style={{ fontSize: 13, fontWeight: 700, color: 'var(--af-t1)' }}>Comparaison doublons</div>
          <div style={{ fontSize: 11, color: 'var(--af-t3)' }}>Groupe {gIdx + 1} sur {groups.length}</div>
        </div>
      ...
      {/* Side-by-side */}
      <div style={{ flex: 1, display: 'flex', gap: 10, padding: 14, overflow: 'hidden' }}>
        {[pA, pB].map((photo, i) => {
          ...
          return (
            <div key={photo.id} onClick={() => handleKeep(photo.id)} style={{
              ...
              {/* Photo */}
              <div style={{
                flex: 1, borderRadius: 10, position: 'relative', overflow: 'hidden', minHeight: 180,
                ...
                {isBetter && (
                  <div style={{
                    ...
                    padding: '4px 12px', background: 'var(--af-review)', borderRadius: 20,
                    fontSize: 11, fontWeight: 800, color: '#000',
                  }}>IA recommande ★</div>
                )}
                <div style={{
                  position: 'absolute', top: 10, right: 10,
                  padding: '4px 10px', background: 'rgba(0,0,0,0.65)', backdropFilter: 'blur(6px)',
                  borderRadius: 7, fontSize: 18, fontWeight: 800, color: sc,
                }}>{photo.score}</div>
                <div style={{
              ...
              <div style={{ padding: '0 2px' }}>
                {([['Nettete', photo.sharp], ['Exposition', photo.expo], ['Composition', photo.comp]] as [string, number][]).map(([l, v]) => {
                  const c = v >= 80 ? 'var(--af-pick)' : v >= 60 ? 'var(--af-review)' : 'var(--af-reject)';
                  return (
      ...
      {/* Keyboard hints */}
      <div style={{
        padding: '3px 0 8px', display: 'flex', gap: 14, justifyContent: 'center', flexShrink: 0,
      }}>
