...
function elementIsDescription(type: unknown): boolean {
  ...
  if (type === DialogDescription || type === DialogPrimitive.Description)
    return true;
...
>(({ className, children, description, ...props }, ref) => {
  ...
  const hasOwnDescription = React.useMemo(
    () => subtreeHasDescription(children),
    [children]
  );
  // On n'injecte un fallback que si rien d'autre ne décrit le dialog.
  const renderFallback =
    !description && !props['aria-describedby'] && !hasOwnDescription;
  ...
  const describedBy =
    ...
    (shouldRenderDescription ? generatedId : undefined);
  const contentProps = describedBy
  ...
  return (
    <DialogPortal>
      ...
      <DialogPrimitive.Content
        ref={ref}
        className={cn(
          'data-[state=open]:animate-in data-[state=closed]:animate-out data-[state=closed]:fade-out data-[state=open]:fade-in data-[state=open]:slide-in-from-bottom-2 fixed left-1/2 top-1/2 z-50 grid w-full max-w-xl translate-x-[-50%] translate-y-[-50%] gap-4 overflow-hidden rounded-lg border bg-popover p-6 text-popover-foreground shadow-lg duration-200',
          className
        )}
        {...contentProps}
      >
