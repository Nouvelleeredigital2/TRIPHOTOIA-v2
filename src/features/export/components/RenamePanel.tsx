...
const TOKENS = [
  ...
  { token: '{label}', label: 'Label', title: 'Label couleur (red, yellow…)' },
  {
    token: '{pick}',
    label: 'Pick',
    title: '"pick" si la photo est marquée Pick, sinon vide',
  },
];
    ...
    <div className="space-y-3">
        ...
        <div className="flex flex-wrap gap-1.5">
          {TOKENS.map(({ token, label, title }) => (
            <button
              ...
              className="rounded border border-border bg-muted px-2 py-0.5 font-mono text-xs transition-colors hover:border-primary hover:bg-primary hover:text-primary-foreground"
            >
          ...
          {pattern && (
            <button
              ...
              className="rounded border border-destructive/40 px-2 py-0.5 text-xs text-destructive transition-colors hover:bg-destructive/10"
            >
              Effacer
            </button>
          )}
      ...
      {previews.length > 0 && (
        <div className="space-y-1">
          ...
          <div className="divide-y divide-border/30 rounded-md border border-border/50 bg-muted/30">
            {previews.map((photo, i) => (
              ...
              >
                <span
                  className="max-w-[35%] truncate text-muted-foreground"
                ...
                <span
                  ...
                  title={expandPreview(pattern, photo, i)}
                >
