...
export function buildDuplicatePhotoIds(photos: Photo[], duplicateGroups: DuplicateGroup[]) {
  const activePhotoIds = new Set(photos.map((photo) => photo.id));

  return new Set(
    duplicateGroups
      .filter((group) => group.photos.some((photo) => activePhotoIds.has(photo.id)))
...
export function buildPhotosToExport({
  ...
  const analyzedPhotos = photos.filter((photo) => photo.analysis && !photo.analysis.error);
  const duplicatePhotoIds = buildDuplicatePhotoIds(analyzedPhotos, duplicateGroups);

  return analyzedPhotos.filter((photo) => {
    const isRejected = rejectedPhotoIds.has(photo.id) || photo.analysis?.isRejected === true;
    ...
    if (options.filterMode === 'min-rating' && (photo.analysis?.rating ?? 0) < options.minRating) return false;
    return true;
  });
}
