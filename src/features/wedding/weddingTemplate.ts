  ...
  [
    {
      ...
      name: 'Préparatifs',
      description:
        'Préparatifs, détails de début de journée et ambiance avant cérémonie.',
    },
    {
    ...
    },
    {
      ...
      name: 'Détails',
      description:
        'Décoration, lieu, alliances, robe, fleurs et éléments éditoriaux.',
    },
    {
      id: 'wedding-soiree',
      name: 'Soirée',
...
export function buildWeddingCollectionDefinitions(
  existingNames = new Set<string>()
) {
  const normalizedExistingNames = new Set(
    Array.from(existingNames).map((name) =>
      normalizeWeddingCollectionName(name)
    )
  );
  ...
  return WEDDING_COLLECTION_TEMPLATE.filter(
    (collection) =>
      !normalizedExistingNames.has(
        normalizeWeddingCollectionName(collection.name)
      )
  );
}
