...
export class GPURetouchProcessor {
   ...
   */
  async applyRetouch(image: HTMLImageElement, options: RetouchOptions): Promise<HTMLCanvasElement> {
    ...
    if (this.webgl.isSupported) {
      try {
        ...
        console.warn('[GPURetouchProcessor] WebGL path failed, falling back to Canvas:', err);
      }
    }
  ...
  private initializeWorker(): void {
    ...
    try {
      this.worker = new Worker(new URL('../../workers/retouchWorker.ts', import.meta.url), {
        type: 'module',
      });
      this.worker.addEventListener('message', this.handleWorkerMessage);
      this.worker.addEventListener('error', this.handleWorkerError);
    } catch (error) {
      console.warn('Retouch worker not available, falling back to main thread processing.', error);
      this.worker = null;
    }
  }

  private handleWorkerMessage = (event: MessageEvent<RetouchWorkerResponse>) => {
    ...
    if (response.type === 'SUCCESS') {
      if (pending.kind !== 'PROCESS') {
        pending.reject(new Error('Incohérence de type de requête process/auto.'));
        return;
      }
      try {
    ...
    if (response.type === 'AUTO_SUCCESS') {
      if (pending.kind !== 'AUTO') {
        pending.reject(new Error('Incohérence de type de requête auto/process.'));
        return;
      }
  ...
  private handleWorkerError = (event: ErrorEvent) => {
    this.pendingWorkerRequests.forEach(({ reject }) => {
      const reason = event.error instanceof Error ? event.error : new Error(event.message);
      reject(reason);
    });
  ...
  private async runRetouchPipeline(imageData: ImageData, options: RetouchOptions): Promise<ImageData> {
    try {
      const workerResult = await this.processImageDataWithWorker(imageData, options);
      if (workerResult) {
        return workerResult;
      }
      ...
      console.warn('Erreur du worker de retouche, traitement sur le thread principal.', error);
      const fallbackData = this.ctx.getImageData(0, 0, imageData.width, imageData.height);
  ...
  private async processImageDataWithWorker(
    ...
    return new Promise<ImageData>((resolve, reject) => {
      ...
      try {
        this.worker!.postMessage(payload satisfies RetouchWorkerRequestPayload, [payload.data]);
      } catch (error) {
  ...
  private async processAutoPresetWithWorker(imageData: ImageData): Promise<AutoRetouchPreset | null> {
    if (!this.worker) {
      this.initializeWorker();
    }
    ...
    try {
      ...
      console.warn('Auto preset worker failed, falling back to main thread.', error);
      return null;
    }
  }

  async computeAutoRetouchPreset(image: HTMLImageElement): Promise<AutoRetouchPreset> {
    const { width, height } = image;
    if (width === 0 || height === 0) {
  ...
  private processImageDataOnMainThread(imageData: ImageData, options: RetouchOptions): ImageData {
  ...
  private buildFilterString(options: RetouchOptions): string {
    ...
    const saturationFactor = clamp(1 + (options.saturation + options.vibrance * 0.6) / 100, 0, 4);
    if (Math.abs(options.saturation) > 0.01 || Math.abs(options.vibrance) > 0.01) {
      filters.push(`saturate(${saturationFactor})`);
    }

    if (options.temperature !== 0 || options.tint !== 0) {
      const hueRotate = clamp(options.tint * 0.6 + options.temperature * 0.25, -180, 180);
      if (hueRotate !== 0) {
   ...
   */
  async applyHistogramEqualization(image: HTMLImageElement): Promise<HTMLCanvasElement> {
    const { width, height } = image;
    this.canvas.width = width;
    this.canvas.height = height;
   ...
   */
  async applyGrayWorldCorrection(image: HTMLImageElement): Promise<HTMLCanvasElement> {
    const { width, height } = image;
    this.canvas.width = width;
    this.canvas.height = height;
  ...
  private grayWorldCorrection(imageData: ImageData): ImageData {
    ...
    for (let i = 0; i < data.length; i += 4) {
      ...
      output.data[i + 1] = Math.min(Math.max(Math.round(data[i + 1] * scaleG), 0), 255);
      output.data[i + 2] = Math.min(Math.max(Math.round(data[i + 2] * scaleB), 0), 255);
