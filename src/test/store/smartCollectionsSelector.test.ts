...
const collectionIds = (
  ...
  resolveSmartCollections(photos, duplicateGroups, rejectedPhotoIds).find(
...
describe('smartCollectionsSelector', () => {
  it('defines the system smart collections required by Sprint 13', () => {
    expect(
      SMART_COLLECTIONS.slice(0, 9).map((collection) => collection.id)
    ).toEqual([
      'sc-all',
      'sc-unreviewed',
      'sc-review',
  ...
  it('keeps system collections updated from photo decisions and analysis', () => {
    ...
    const blurry = makePhoto('blurry', {
      analysis: { isPick: true, isBlurry: true },
    });
    ...
    expect(collectionIds('sc-all', photos, duplicateGroups)).toEqual(
      photos.map((photo) => photo.id)
    );
    expect(collectionIds('sc-unreviewed', photos, duplicateGroups)).toEqual([
      'unreviewed',
    ]);
    expect(
      collectionIds(
    ...
    expect(collectionIds('sc-picks', photos, duplicateGroups)).toEqual([
      'pick',
      'favorite',
      'blurry',
      'duplicate-a',
    ...
    expect(collectionIds('sc-favorites', photos, duplicateGroups)).toEqual([
      'favorite',
      'duplicate-a',
    ]);
    expect(
    ...
    ).toEqual(['rejected', 'external-reject']);
    expect(collectionIds('sc-duplicates', photos, duplicateGroups)).toEqual([
      'duplicate-a',
      'duplicate-b',
    ]);
    expect(collectionIds('sc-blurry', photos, duplicateGroups)).toEqual([
      'blurry',
    ]);
    expect(collectionIds('sc-ready-export', photos, duplicateGroups)).toEqual([
      'pick',
      'favorite',
    ]);
  });
  ...
  it('matches duplicate and export-ready rules with duplicate context', () => {
    const duplicate = makePhoto('duplicate', {
      analysis: { isPick: true, rating: 5 },
    });
    ...
    expect(
      matchesRule(duplicate, { type: 'isDuplicate' }, { duplicateGroups })
    ).toBe(true);
    expect(
      matchesRule(standalone, { type: 'isDuplicate' }, { duplicateGroups })
    ).toBe(false);
    expect(
      matchesRule(duplicate, { type: 'readyToExport' }, { duplicateGroups })
    ).toBe(false);
    expect(
      matchesRule(standalone, { type: 'readyToExport' }, { duplicateGroups })
    ).toBe(true);
  });
});
