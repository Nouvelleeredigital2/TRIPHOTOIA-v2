...
const migrationPath = join(process.cwd(), 'supabase', 'migrations', '20260529140000_treephoto_faces_v2.sql');

describe('TreePhoto faces schema migration', () => {
  const sql = readFileSync(migrationPath, 'utf8').toLowerCase();

  it('adds a strict opt-in flag defaulting to false', () => {
    expect(sql).toContain('add column if not exists face_analysis_enabled boolean not null default false');
  });
  ...
  it('creates the photo_faces table with a 128-dim face embedding', () => {
    ...
    expect(sql).toContain('person_id uuid references public.people(id) on delete set null');
